...
//! to UDP.
// Обновленные/добавленные импорты для TLS
use std::sync::Arc;
use tokio_rustls::{TlsAcceptor, server::TlsStream}; // Предполагается, что tokio-rustls в зависимостях
...
pub struct Options {
    ...
    // Новое поле для опционального TlsAcceptor
    /// Optional TLS acceptor for encrypting incoming TCP connections.
    #[cfg_attr(feature = "clap", arg(skip))] // Скрыто от clap, будет передаваться программно
...
impl fmt::Display for Tcp2UdpError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            // Форматирование новой ошибки
            TlsHandshake(_) => "TLS handshake failed".fmt(f),
        }
    }
}
...
pub async fn run(options: Options) -> Result<Infallible, Tcp2UdpError> {
    ...
    // Передача tls_acceptor в локальную переменную для использования в цикле
    ...
    for tcp_listen_addr in options.tcp_listen_addrs {
        ...
        // Передача клона tls_acceptor в каждую задачу
        ...
        join_handles.push(tokio::spawn(async move {
            process_tcp_listener(
                ...
                tcp_nodelay,
                statsd,
                tls_acceptor_task, // Передаем tls_acceptor в обработчик
            )
            .await;
        }));
    }
...
// Обновленная сигнатура функции для приема tls_acceptor
async fn process_tcp_listener(
    ...
    tls_acceptor: Option<Arc<TlsAcceptor>>, // Новый аргумент
) -> ! {
    let mut cooldown =
    ...
    loop {
        match tcp_listener.accept().await {
            Ok((tcp_stream, tcp_peer_addr)) => {
                ...
                // Передача клона tls_acceptor в задачу обработки соединения
                let tls_acceptor_conn = tls_acceptor.clone();

                tokio::spawn(async move {
                    statsd.incr_connections();
                    // Вызов обновленной функции process_socket с потенциальным TLS-потоком
                    if let Err(error) = process_socket(
                        ...
                        udp_forward_addr,
                        tcp_recv_timeout,
                        tls_acceptor_conn, // Передаем tls_acceptor в process_socket
                    )
                    .await
                    {
                        log::error!("Error: {}", error.display("\nCaused by: "));
...
/// If `tls_acceptor` is provided, performs TLS handshake on `tcp_stream` before forwarding.
// Обновленная сигнатура для работы с обобщенным потоком
async fn process_socket<S>(
    tcp_stream: S, // Обобщенный тип потока
...
where
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send + 'static, // Ограничения на обобщенный тип
{
    // Выполнение TLS handshake, если предоставлен acceptor
    let tcp_stream = if let Some(acceptor) = tls_acceptor {
        let tls_stream = acceptor.accept(tcp_stream).await.map_err(Tcp2UdpError::TlsHandshake)?;
        // Логирование успешного TLS handshake (опционально)
        log::debug!("TLS handshake successful for connection from {}/TCP", Redact(tcp_peer_addr));
        Box::new(tls_stream) as Box<dyn tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send>
    } else {
        Box::new(tcp_stream) as Box<dyn tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send>
    };
    ...
    // Передача обработанного (возможно, TLS) потока в forward_traffic
